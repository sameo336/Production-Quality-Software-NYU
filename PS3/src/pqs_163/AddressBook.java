...
/**
 * Creates and empty address book which can be used to create contacts with fields name, phone
 * number, postal address, email address and note. Also has the functionality to save the address
 * book to a file and read from a file with appropriate formatting.
 * 
...
public class AddressBook {
  ...
  /**
   * Default constructor for the address book class. Instantiates the list containing entries of the
   * address book
   * 
   */
  public AddressBook() {
   ...
   * 
   * @return <code>true</code> if the entry is removed successfully; <code>false</code> otherwise.
   * 
   */
   ...
   * 
   * @return <code>true</code> if the contact is added successfully; <code>false</code> otherwise.
   * 
   */
  ...
  public boolean addNewContact(String name, String phoneNumber, String postalAddress,
    ...
    AddressBookEntry newEntry =
        new AddressBookEntry(name, phoneNumber, postalAddress, emailAddress, note);
  ...
  /**
   * Searches the entire address book by the given query. Tries to find contacts which contain the
   * exact given query, partial matches are ignored.
   * 
  ...
  /**
   * Saves the current address book to a file. If no file is found with the given path, a new file
   * is created. If there exists a file with the same name, it is overwritten. Conversion from book
   * entry list to appropriate format is handled here.
   * 
   * @param pathToFile The path to save the file at
   * 
   * @throws IOException If an error is occurred during writing into the file, an exception is
   *           thrown
   * 
   */
  ...
  /**
   * Reads from a file with appropriate formatting and handles conversion to take into account new
   * line characters in fields.
   * 
   * @param pathToFile absolute or the relative address of the file. If file is not found, an
   ...
   * @throws StringIndexOutOfBoundsException thrown if an error occurs during reading data from the
   *           file. Most likely to occur if the address book is changed without using saveToFile
   *           method
   * 
   * 
   */

  public AddressBook readFromFile(String pathToFile)
  ...
  ArrayList<AddressBookEntry> getListOfEntries_forTest() {
